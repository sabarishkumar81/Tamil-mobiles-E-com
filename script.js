...
// Load all users (fetch from server, fallback to local auth)
async function loadUsers() {
    try {
        const res = await fetch('/api/users?limit=500');
        if (!res.ok) throw new Error('Fetch failed');
        const data = await res.json();
        // Normalize id field used elsewhere
        allUsers = data.users.map(u => ({ id: u._id, name: u.name || '', email: u.email || '', phone: u.phone || '', createdAt: u.createdAt || new Date(), updatedAt: u.updatedAt || new Date(), role: u.role || 'user' }));
        filteredUsers = [...allUsers];
        renderUsers();
    } catch (err) {
        console.warn('API fetch failed, falling back to local auth', err);
        allUsers = (auth.getUsers && auth.getUsers()) || [];
        filteredUsers = [...allUsers];
        renderUsers();
    }
}
...
// Delete user with authentication check (calls server)
...
// Create a user from admin UI (simple prompt-based helper)
async function createUser() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            showMessage(err.error || 'Create failed', 'error');
            return;
        }
        showMessage('User created/updated successfully', 'success');
        await loadUsers();
        updateStats();
    } catch (err) {
        console.error(err);
...
function updateStats() {
    // Try to compute basic stats from allUsers
    const total = allUsers.length;
    const now = new Date();
    const today = allUsers.filter(u => new Date(u.createdAt).toDateString() === now.toDateString()).length;
    const month = now.getMonth();
    const newThisMonth = allUsers.filter(u => new Date(u.createdAt).getMonth() === month).length;

    document.getElementById('adminTotalUsers').textContent = total;
    document.getElementById('adminNewUsers').textContent = newThisMonth;
    document.getElementById('adminTodayUsers').textContent = today;
}
